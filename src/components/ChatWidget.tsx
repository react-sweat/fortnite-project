...
export default function ChatWidget() {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
            {isOpen && (
                <div className="mb-4 w-80 md:w-96 h-96 bg-white rounded-2xl shadow-2xl border border-slate-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-300">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
                        ...
                        {messages
                            .filter(msg => msg.role !== 'system' && !(msg.content || '').includes('[[get_stats:'))
                            .map((msg, idx) => (
                                <div
                                    key={idx}
                    ...
                    <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-slate-100">
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 bg-slate-100 text-slate-900 text-sm rounded-full px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
                            />
                            <button
                                type="submit"
